...
def get_realtime_bid_ask(symbol: str = "600519") -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        try:
            ...
            print(f"代理连接错误，请检查网络代理设置或尝试关闭代理")
            print(f"提示：可以尝试设置环境变量 NO_PROXY=* 或关闭系统代理")
            return None
...
def get_money_flow_data(symbol: str = "600519", market: str = None) -> Optional[pd.DataFrame]:
    """
    获取资金流向数据（主力/大单/中单/小单）

    Args:
        symbol: 股票代码
        market: 市场代码，"sh"(上海)或"sz"(深圳)，如果为None则自动判断
    ...
    try:
        ...
        # 使用 stock_individual_fund_flow 获取单个股票的资金流向历史数据
        # 参数: stock=股票代码, market=市场代码(sh/sz)
...
def get_money_flow_rank(indicator: str = "今日") -> Optional[pd.DataFrame]:
    """
    获取全市场资金流向排名数据

    Args:
        indicator: 时间周期，可选值："今日"、"3日"、"5日"、"10日"
    ...
    try:
        ...
        print(f"正在获取全市场资金流向排名数据（{indicator}）...")
