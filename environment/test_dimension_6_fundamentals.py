...
def get_financial_statements(symbol: str = "600519") -> Optional[dict]:
    ...
    try:
        ...
        # 1. 资产负债表
        try:
            balance_sheet = ak.stock_financial_abstract(symbol=symbol, indicator="资产负债表")
        ...
        # 2. 利润表
        try:
            income_statement = ak.stock_financial_abstract(symbol=symbol, indicator="利润表")
        ...
        # 3. 现金流量表
        try:
            cash_flow = ak.stock_financial_abstract(symbol=symbol, indicator="现金流量表")
...
def get_dividend_data(symbol: str = "600519") -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        if df is not None and not df.empty:
            # 计算股息率等指标
            if '每股派息' in df.columns and '收盘价' in df.columns:
                df['股息率'] = (df['每股派息'] / df['收盘价']) * 100
...
def get_roe_roa_data(symbol: str = "600519") -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        # 提取ROE、ROA相关列
        if df is not None and not df.empty:
            roe_cols = [col for col in df.columns if 'ROE' in col or '净资产收益率' in col or
                       'ROA' in col or '资产收益率' in col]
            if roe_cols:
                return df[['日期'] + roe_cols] if '日期' in df.columns else df[roe_cols]
...
def get_debt_metrics(symbol: str = "600519") -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        # 提取偿债能力相关列
        if df is not None and not df.empty:
            debt_cols = [col for col in df.columns if any(x in col for x in
                        ['资产负债率', '流动比率', '速动比率', '现金比率', '利息保障倍数'])]
            if debt_cols:
                return df[['日期'] + debt_cols] if '日期' in df.columns else df[debt_cols]
...
def get_growth_metrics(symbol: str = "600519") -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        # 提取成长性相关列
        if df is not None and not df.empty:
            growth_cols = [col for col in df.columns if any(x in col for x in
                          ['增长', '增长率', 'EPS', '营业收入', '净利润'])]
            if growth_cols:
                return df[['日期'] + growth_cols] if '日期' in df.columns else df[growth_cols]
...
def get_operating_metrics(symbol: str = "600519") -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        # 提取运营能力相关列
        if df is not None and not df.empty:
            operating_cols = [col for col in df.columns if any(x in col for x in
                             ['周转', '周转率', '周转天数', '存货', '应收账款'])]
            if operating_cols:
                return df[['日期'] + operating_cols] if '日期' in df.columns else df[operating_cols]
